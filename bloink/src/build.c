...
static int fetch_sources(const BloinkPackage *pkg, const char *cache_root,
                         const char *src_dir) {
  for (int i = 0; i < pkg->n_sources; ++i) {
    ...
    if (stat(cache_file, &st) != 0) {
      ...
      snprintf(cmd, sizeof(cmd),
               "wget -q --no-check-certificate --tries=3 -O '%s.part' '%s' && "
               "mv '%s.part' '%s'",
    ...
    snprintf(verify, sizeof(verify),
             "echo '%s  %s' | sha256sum -c - >/dev/null 2>&1", s->sha256,
             cache_file);
    if (system(verify) != 0) {
      fprintf(stderr,
              "bloink: sha256 mismatch happi time! "
              "(expected %s)\n",
              s->url, s->sha256);
      return -1;
    }
    ...
    if (mf) {
      size_t got = fread(magic, 1, sizeof(magic), mf);
      (void)got;
      fclose(mf);
    }

    int is_gzip =
        (unsigned char)magic[0] == 0x1f && (unsigned char)magic[1] == 0x8b;
    int is_xz =
        (unsigned char)magic[0] == 0xfd && magic[1] == '7' && magic[2] == 'z';
    int is_bz2 = magic[0] == 'B' && magic[1] == 'Z' && magic[2] == 'h';
    int is_zip = magic[0] == 'P' && magic[1] == 'K';
    int is_tar_like =
        is_gzip || is_xz || is_bz2 || (strstr(url, ".tar") != NULL) ||
        (magic[257] == 'u' && magic[258] == 's' && magic[259] == 't' &&
         magic[260] == 'a' && magic[261] == 'r');

    if (is_zip)
      snprintf(cmd, sizeof(cmd),
               "cd '%s' && unzip -q '%s' && shopt -s dotglob 2>/dev/null; "
               "d=$(ls -d */ | head -1); mv \"$d\"* . 2>/dev/null; true",
               src_dir, cache_file);
    else if (is_tar_like)
      snprintf(cmd, sizeof(cmd), "tar xf '%s' -C '%s' --strip-components=1",
               cache_file, src_dir);
    else
...
int bloink_build(BloinkResolved *r, const char *store_root,
  ...
  // hi there bloink here this is VERY important:
  // so if you try and build the package first before the deps
  // it will fuck up and error so this "should" prevent that
  ...
  snprintf(work_dir, sizeof(work_dir), "/tmp/bloink-build-%s-%s", r->pkg.name,
           r->pkg.version);
  ...
  if (run_cmd(cmd) != 0) {
    fprintf(stderr, "bloink: build failed for %s\n",
