...
static void copy_field_str(lua_State *L, const char *key, char *dst,
                           size_t dstlen) {
...
static int load_sources(lua_State *L, BloinkPackage *pkg) {
  ...
  for (int i = 1; i <= n && pkg->n_sources < BLOINK_MAX_SOURCES; ++i) {
    ...
    if (lua_istable(L, -1)) {
      ...
      if (s->sha256[0] == 0) {
        fprintf(stderr,
                "bloink: source %s has no sha256 — refusing ",
                s->url);
        lua_pop(L, 1);
        lua_pop(L, 1);
        return -1;
...
static int load_build_steps(lua_State *L, BloinkPackage *pkg) {
  ...
  for (int i = 1; i <= n && pkg->n_steps < BLOINK_MAX_STEPS; ++i) {
    lua_rawgeti(L, -1, i);
    if (lua_isstring(L, -1)) {
      snprintf(pkg->steps[pkg->n_steps], sizeof(pkg->steps[0]), "%s",
               lua_tostring(L, -1));
      pkg->n_steps++;
    }
    lua_pop(L, 1);
