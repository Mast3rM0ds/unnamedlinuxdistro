// self explaintory name right here
// it resolves your packages.
#include "bloink.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static int recipe_path_for(const char *recipe_dir, const char *name, char *out,
                           size_t outlen) {
...
static BloinkResolved *resolve_one(const char *recipe_dir,
                                   const char *store_root, const char *name) {
  ...
  for (int i = 0; i < node->pkg.n_deps; ++i) {
    BloinkResolved *d =
        resolve_one(recipe_dir, store_root, node->pkg.deps[i].name);
    if (!d) {
      g_n_visiting--;
      return NULL;
    }
  ...
  bloink_store_path(&node->pkg, dep_hashes_buf, store_root, node->store_path,
                    sizeof(node->store_path));
...
int bloink_resolve(const char *recipe_dir, const char *store_root,
                   const char *name, BloinkResolved **out_list,
                   int *out_count) {
  g_n_nodes = 0;
  g_n_visiting = 0;
