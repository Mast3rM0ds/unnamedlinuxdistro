...
verify_boot()
{
	if ! yesno "$RC_USER_SERVICES" && [ ! -e ${RC_SVCDIR}/softlevel ]; then
...
sourcex "/usr/libexec/rc/sh/functions.sh"
sourcex "/usr/libexec/rc/sh/rc-functions.sh"
case $RC_SYS in
	PREFIX|SYSTEMD-NSPAWN) ;;
	*) yesno "$RC_USER_SERVICES" || sourcex -e "/usr/libexec/rc/sh/rc-cgroup.sh";;
esac
...
for _c in $_c; do
	for _f in $_conf_d; do
		if ! sourcex -e "$_f/conf.d/$_c.$RC_RUNLEVEL"; then
			sourcex -e "$_f/conf.d/$_c"
		fi
	done
done
...
sourcex "/usr/libexec/rc/sh/runit.sh"
sourcex "/usr/libexec/rc/sh/s6.sh"
sourcex "/usr/libexec/rc/sh/start-stop-daemon.sh"
sourcex "/usr/libexec/rc/sh/supervise-daemon.sh"
...
for _cmd in describe start stop status ${extra_commands:-$opts} $extra_started_commands $extra_stopped_commands; do
	[ "$_cmd" = "$1" ] || continue
	_func="$_cmd"
...
if ! [ "$_func" ]; then
	eerror "$RC_SVCNAME: command '$1' not a default nor extra command"
	exit 1
fi

# If we're in the background, we may wish to fake some commands. We do this so we can
# "start" ourselves from inactive which then triggers other services to start which
# depend on us. A good example of this is openvpn.
...
for _cmd in $extra_stopped_commands; do
	...
	if verify_boot && ! service_stopped; then
		eerror "$RC_SVCNAME: cannot '$_func' as it has not been stopped"
		exit 1
	fi
done
...
[ "$(command -v "${_func}_pre")" = "${_func}_pre" ] && { "${_func}_pre" || exit $?; }
"$_func" || exit $?
[ "$(command -v "${_func}_post")" = "${_func}_post" ] && { "${_func}_post" || exit $?; }

[ "$_func" = start ] && [ -n "$RC_REEXPORT" ] && service_export $RC_REEXPORT
[ "$(command -v cgroup_cleanup)" = cgroup_cleanup ] && [ "$_func" = stop ] && yesno "${rc_cgroup_cleanup}" && cgroup_cleanup
[ "$(command -v cgroup2_remove)" = cgroup2_remove ] && { [ "$_func" = stop ] || [ -z "${command}" ]; } && cgroup2_remove
