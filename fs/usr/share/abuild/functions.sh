...
arch_to_triplet() {
	...
	aarch64_be)	echo "aarch64_be-alpine-linux-musl" ;;
...
triplet_to_arch() {
	...
	x86_64-*-*-*)		echo "x86_64" ;;
	*)
		echo "Couldn't determine arch from triplet: $1" >&2
		exit 1
		;;
	esac
}
...
triplet_to_libc() {
	...
	*-*-*-gnu*)	echo "glibc" ;;
	*)
		echo "Couldn't determine libc from triplet: $1" >&2
		exit 1
		;;
	esac
}
...
readconfig() {
	local _APORTSDIR _REPODEST _SRCDEST _ABUILD_USERDIR
	local _CC=cc _CXX=c++ _LD=ld _CARCH _CHOST _CTARGET _CPPFLAGS _CFLAGS _CXXFLAGS _LDFLAGS
	local _MAKEFLAGS _PACKAGER _USE_COLORS _USE_CCACHE
	...
	if [ -n "${_ABUILD_USERDIR+x}" ] || [ -n "$ABUILD_USERDIR" ]; then
		ABUILD_USERDIR=${_ABUILD_USERDIR-$ABUILD_USERDIR}
	elif ! [ -d "${XDG_CONFIG_HOME:-$HOME/.config}/abuild" ] && [ -d "$HOME/.abuild" ]; then
		ABUILD_USERDIR=$HOME/.abuild
	else
		ABUILD_USERDIR=${XDG_CONFIG_HOME:-$HOME/.config}/abuild
	fi
	...
	[ "$(arch_to_triplet $CBUILD)" != "unknown" ] && CBUILD="$(arch_to_triplet $CBUILD)"
	[ "$(arch_to_triplet $CHOST)" != "unknown" ] && CHOST="$(arch_to_triplet $CHOST)"
	[ "$(arch_to_triplet $CTARGET)" != "unknown" ] && CTARGET="$(arch_to_triplet $CTARGET)"
	if [ -z "$CBUILD" ]; then
		echo "Unable to deduce build architecture. Install apk-tools, or set CBUILD." >&2
		exit 1
	fi

	[ -z "$CARCH" ] && CARCH="$(triplet_to_arch $CHOST)"
	[ -z "$CLIBC" ] && CLIBC="$(triplet_to_libc $CHOST)"
	[ -z "$CBUILD_ARCH" ] && CBUILD_ARCH="$(triplet_to_arch $CBUILD)"
	[ -z "$CTARGET_ARCH" ] && CTARGET_ARCH="$(triplet_to_arch $CTARGET)"
	[ -z "$CTARGET_LIBC" ] && CTARGET_LIBC="$(triplet_to_libc $CTARGET)"
